...
export default function select_position(id,name) {
    let data = {
        'position_id' : id
    }
    console.log(data)
    axios.post('select_position', Qs.stringify(data))
        .then(res => {
            const result =res['data']
            let str = "<h1>"+name+"</h1><table>";
            str +=
            `<tr style="background-color: #FFFFFF;">
            <td>編號</td><td>名子</td><td>性別</td><td>年齡</td><td>國籍</td><td>縣市</td><td>地區</td><td colspan="2" ><center><button id='doinsert'>新增</button></center></td>
            </tr>`
